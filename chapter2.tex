...
So the uncertainty size needs be less than about 13.5\degree\ on average to make
the segment length trustworthy. Therefore, to a certain extent, the ``Fit
...
530 Myr paths, and sequential 100-130 Myr subsections were calculated for path
pairs with (Fig.~\ref{fig:ssperc}) and without (Fig.~\ref{fig:sspercni}) poles
with zero spatial uncertainties at 350 Ma, 360 Ma, 380 Ma, 390 Ma, 450 Ma, 460
Ma and 520 Ma calculated using linear interpolation by~\cite{T12}.
...
450 Ma, 460 Ma and 520 Ma pole coordinates are interpolated~\cite{T12}, and
...
Laurussia calculated using a running mean method by~\cite{T12}, with paleopoles
...
same paleopoles for the APWPs in Figure 13(a) of~\cite{T12}. Azimuthal
...
judiciously applied~\cite[for example]{B10,B16}. A more detailed study might further
constrain the regions, continents and/or time periods where a correction is
appropriate, and those where it is not. But for the purposes of this paper, this
overview is sufficient to demonstrate the potential usefulness of our difference
metrics when considering the effect of different techniques and corrections used
to generate an APWP.
...
PmagPy~\cite{T16} and GMT~\cite{W13}. All images are produced using
GMT~\cite{W13}. We thank 3 anonymous reviewers for suggestions that greatly
...
\paragraph{Hardware Required} Intel(R) Core(TM) i7-6700 CPU @ 3.40GHz or higher;
