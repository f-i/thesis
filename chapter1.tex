...
132\textendash100 Ma~\cite{O05} (Hybrid hotspot model~\cite{Sh12}), combining a
moving hotspot frame from 100\textendash0 Ma~\cite{O05} with a paleomagnetic
...
dipole field)~\cite{T08} from 140\textendash100 Ma (Hybrid paleomagnetic
model~\cite{Sh12}), and combining a moving hotspot frame from 120\textendash100
Ma~\cite{O05} with a True Polar Wander (TPW) corrected paleomagnetic
model~\cite{S08} from 100\textendash0 Ma (Hybrid TPW-corrected
model~\cite{Sh12}).
...
\begin{figure}
    \centering
        \includegraphics[width=0.88\textwidth]{../sphinx/source/I/fig/gs13101.png}
    \captionsetup{width=.95\textwidth}
...
(e.g.\ 2 Myr) to see whether the VGPs in each time interval overlap within their
error ellipses, as they should. Sometimes, this is the case
...
\begin{figure*}[tbp]
  ...
  \begin{subfigure}[htbp]{.49\textwidth}
    ...
    \caption{North America: 64\textendash62 Ma (mean age)}\label{Fig:chap_intro_nac5052agemean}
  \end{subfigure}
  \begin{subfigure}[htbp]{.49\textwidth}
    \captionsetup{skip=0pt}
    \centering
    \includegraphics[width=1.01\linewidth]{../sphinx/source/I/fig/in97.pdf}
    \caption{India: 9\textendash7 Ma (mean age)}\label{Fig:chap_intro_nac2830agemean}
  \end{subfigure}
...
three poles (Fig.~\ref{Fig:chap_intro_nac2830agemean}) are from sedimentary
...
deformation (Fig.~\ref{Fig:chap_intro_nac2830agemean}), and we would ideally
...
100\textendash0 Ma, GPMDB 4.6b has 198 NAC poles, but only 17 for Greenland and
24 for Arabia.
