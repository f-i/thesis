...
limitations to paleomagnetic data such as the poorly-constrained longitudes of
...
spatial uncertainties in pole positions. An additional Fit Quality metric is
used to discriminate between low difference scores caused by comparing poorly
constrained paths with large spatial uncertainties from those caused by a close
fit between well-constrained paths. The individual and combined metrics are
demonstrated using tests on synthetic pairs of APWPs with varying degrees of
spatial and geometric difference. In a test on real paleomagnetic data, these
metrics can quantify the effects of correction for inclination shallowing in
sedimentary rocks on Gondwana and Laurussia's 320–0 Ma APWPs. A Python package
on GitHub is provided online as an open-source software to allow automatically
