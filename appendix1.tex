...
overlap~\cite{T91}. The 95\% confidence bounds of the Cartesian coordinates of
...
overlap, the poles are indistinguishable~\cite{T91}. The actual test used is
dependent on the number of paleopoles ($\mathbf{N}$) used to calculate the mean
pole in the APWP$\colon$

\begin{itemize}
\item when $\mathbf{N}>25$ a simple bootstrap~\cite{T91} generates a
...
\item when $1<\mathbf{N}\leq25$ a parametric bootstrap~\cite{T91} generates a
...
sphere is a complicated problem~\cite{K82}. An analogue approach is proposed
...
``multivariate\_normal''~\cite{W11}. The lengths of the uncertainty ellipse's
...
star in Fig.~\ref{fig:ellip_sim}), an Euler rotation~\cite{G99} (black star and
